...
import {
  ...
  Sun, Moon, Globe, Sparkles, Play, TrendingUp, MessageCircle,
  AlertCircle, User, AtSign, Calendar, ArrowRight, KeyRound,
} from 'lucide-react';
...
const ALL_COUNTRIES_NAMES = [
  ...
  'Canada', 'Argentina', 'Chile', 'Colombia', 'Peru', 'Venezuela',
  'Bangladesh', 'Philippines', 'Myanmar', 'Cambodia', 'Laos',
...
export default function SignupPage() {
  ...
  // Social signup
  const [socialSetup, setSocialSetup] = useState(false);
  const [socialUsername, setSocialUsername] = useState('');
  const [socialUsernameStatus, setSocialUsernameStatus] = useState<UsernameStatus>('idle');
  ...
  useEffect(() => {
    ...
    if (username.length < 3 || username.length > 30) { setUsernameStatus('invalid'); return; }
    if (/^[._]/.test(username)) { setUsernameStatus('invalid'); return; }
    if (!/^[a-z0-9._]+$/i.test(username)) { setUsernameStatus('invalid'); return; }
  ...
  // Social username check
  useEffect(() => {
    if (!socialUsername) { setSocialUsernameStatus('idle'); return; }
    if (socialUsername.length < 3 || socialUsername.length > 30) { setSocialUsernameStatus('invalid'); return; }
    if (/^[._]/.test(socialUsername)) { setSocialUsernameStatus('invalid'); return; }
    if (!/^[a-z0-9._]+$/i.test(socialUsername)) { setSocialUsernameStatus('invalid'); return; }
    ...
    const timer = setTimeout(async () => {
      const { data } = await supabase
        .from('profiles')
        .select('id')
        .eq('username', socialUsername.toLowerCase())
        .maybeSingle();
      setSocialUsernameStatus(data ? 'taken' : 'available');
    }, 500);
  ...
  function validateFullName(name: string): string | null {
    if (name.length < 2) return t('signup.fullNameMin');
    if (!/^[a-zA-Z\s]+$/.test(name)) return t('signup.fullNameInvalid');
    return null;
  }

  function validateUsername(user: string): string | null {
    if (user.length < 3 || user.length > 30) return t('signup.usernameLength');
    if (/^[._]/.test(user)) return t('signup.usernameStart');
    if (!/^[a-z0-9._]+$/i.test(user)) return t('signup.usernameInvalid');
    return null;
  }

  const pwStrength = checkPasswordStrength(password);
  const strengthScore = Object.values(pwStrength).filter(Boolean).length;
  const strengthLabel = strengthScore <= 1 ? t('login.weak') : strengthScore <= 2 ? t('signup.passwordWeak') : strengthScore <= 3 ? t('login.medium') : t('login.strong');

  async function createProfileAndWallet(userId: string, uname: string, fName: string, userEmail?: string) {
    await supabase
      .from('profiles')
      .update({
  ...
  async function handleEmailSignup(e: React.FormEvent) {
    ...
    if (nameErr) { setError(nameErr); triggerShake(); return; }

    const userErr = validateUsername(username);
    if (userErr) { setError(userErr); triggerShake(); return; }
    if (usernameStatus === 'taken') { setError(t('signup.usernameTaken')); triggerShake(); return; }
    if (usernameStatus === 'checking') { setError(t('signup.checking')); return; }

    const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
    if (!emailRegex.test(email)) { setError(t('signup.emailRegistered')); triggerShake(); return; }
    if (emailStatus === 'taken') { setError(t('signup.emailRegistered')); triggerShake(); return; }

    if (strengthScore < 3) { setError(t('signup.passwordWeak')); triggerShake(); return; }
    if (password !== confirmPassword) { setError(t('signup.passwordNoMatch')); triggerShake(); return; }

    if (!dob) { setError(t('signup.dateOfBirth')); triggerShake(); return; }
    if (getAge(dob) < 13) { setError(t('signup.ageTooYoung')); triggerShake(); return; }

    if (!termsAccepted) { setError(t('signup.termsRequired')); triggerShake(); return; }
    ...
    if (data.user) {
      await createProfileAndWallet(data.user.id, username, fullName, email);
    }
  ...
  async function handleSendOtp() {
    setError('');
    if (!phone || phone.length < 7) { setError(t('signup.phoneSignup')); triggerShake(); return; }
    const nameErr = validateFullName(fullName);
    if (nameErr) { setError(nameErr); triggerShake(); return; }
    const userErr = validateUsername(username);
    if (userErr) { setError(userErr); triggerShake(); return; }
    ...
    const { error: otpError } = await supabase.auth.signInWithOtp({
      phone: fullPhone,
      options: { shouldCreateUser: true, data: { full_name: fullName, username: username.toLowerCase() } },
    });
    setLoading(false);
  ...
  async function handleVerifyOtp() {
    setError('');
    const code = otp.join('');
    if (code.length !== 6) { setError(t('login.enterOtp')); triggerShake(); return; }
    ...
    if (data.user) {
      await createProfileAndWallet(data.user.id, username, fullName);
    }
    if (data.session) {
      await fetchProfile();
      navigate('/onboarding');
  ...
  async function handleSocialSignup(provider: 'google' | 'facebook' | 'apple') {
    setError('');
    setLoading(true);
                ...
                <form onSubmit={handleEmailSignup} className="space-y-4 animate-fadeIn">
                  ...
                  <div className="relative">
                    ...
                    <div className="absolute right-3 top-1/2 -translate-y-1/2">
                      ...
                      {usernameStatus === 'available' && <Check className="h-4 w-4 text-brand-500" />}
                      {usernameStatus === 'taken' && <X className="h-4 w-4 text-coral-500" />}
                      {usernameStatus === 'invalid' && <X className="h-4 w-4 text-coral-500" />}
                    </div>
                  </div>
                  {usernameStatus === 'available' && (
                    <p className="text-xs text-brand-500 -mt-2 ms-1">{t('signup.usernameAvailable')}</p>
                  )}
                  {usernameStatus === 'taken' && (
                    <p className="text-xs text-coral-500 -mt-2 ms-1">{t('signup.usernameTaken')}</p>
                  )}
                  ...
                  <div className="relative">
                    ...
                    <div className="absolute right-3 top-1/2 -translate-y-1/2">
                      ...
                      {emailStatus === 'available' && <Check className="h-4 w-4 text-brand-500" />}
                      {emailStatus === 'taken' && <X className="h-4 w-4 text-coral-500" />}
                    </div>
                  </div>
                  {emailStatus === 'taken' && (
                    <p className="text-xs text-coral-500 -mt-2 ms-1">{t('signup.emailRegistered')}</p>
                  )}
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      aria-label={t('signup.dateOfBirth')}
                      className="w-full pl-10 pr-4 pt-5 pb-1.5 rounded-xl bg-gray-50 dark:bg-navy-300/50 border border-gray-200 dark:border-navy-300 text-gray-900 dark:text-white outline-none focus:border-brand-500 focus:ring-2 focus:ring-brand-500/20 transition-all"
                    />
                  ...
                  {dob && getAge(dob) < 13 && (
                    <p className="text-xs text-coral-500 -mt-2 ms-1">{t('signup.ageTooYoung')}</p>
                  )}
                  ...
                  <label className="flex items-start gap-2.5 cursor-pointer">
                    ...
                    <span className="text-xs text-gray-600 dark:text-gray-400">
                      {t('signup.acceptTerms').split('Terms of Service')[0]}
                      <Link to="/terms" className="text-brand-500 hover:underline" onClick={(e) => e.stopPropagation()}>Terms of Service</Link>
                      {t('signup.acceptTerms').includes('and') ? ' and ' : ' '}
                      <Link to="/privacy" className="text-brand-500 hover:underline" onClick={(e) => e.stopPropagation()}>Privacy Policy</Link>
                    </span>
                  </label>
                  ...
                  {!otpSent ? (
                    <>
                      ...
                      <div className="flex gap-2">
                              ...
                              {COUNTRIES.map((c) => (
                                <button
                                  ...
                                  className={`w-full flex items-center gap-3 px-3 py-2 text-sm text-start transition-colors ${
                                    countryCode === c.code ? 'bg-brand-50 dark:bg-navy-300 text-brand-600 dark:text-brand-400 font-semibold' : 'text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-navy-300'
                                  }`}
                                >
                        ...
                        <div className="relative flex-1">
                          ...
                          <input
                            ...
                            placeholder={t('signup.phoneSignup')}
                      ...
                      <div className="relative">
                        ...
                        <input
                          ...
                          className="w-full pl-10 pr-4 pt-5 pb-1.5 rounded-xl bg-gray-50 dark:bg-navy-300/50 border border-gray-200 dark:border-navy-300 text-gray-900 dark:text-white outline-none focus:border-brand-500 focus:ring-2 focus:ring-brand-500/20 transition-all"
                        />
                        <label className="absolute left-10 top-2 text-xs text-gray-400">{t('signup.dateOfBirth')}</label>
                      </div>

                      <TermsCheckbox t={t} accepted={termsAccepted} onChange={setTermsAccepted} />
                    ...
                    <>
                      ...
                      <div className="text-center text-sm">
                        {resendTimer > 0 ? (
                          <span className="text-gray-400">{t('login.resendIn')} {resendTimer} {t('login.seconds')}</span>
                        ) : (
                          <button onClick={handleSendOtp} className="text-brand-500 hover:underline font-medium">
                            {t('login.resendOtp')}
                          </button>
                        )}
                      </div>
...
function FloatingInput({
  icon, type, label, value, onChange,
}: {
  icon: React.ReactNode; type: string; label: string; value: string; onChange: (v: string) => void;
}) {
  return (
    <div className="relative">
...
function UsernameInput({
  t, username, setUsername, status,
}: {
  t: (key: string) => string; username: string; setUsername: (v: string) => void; status: UsernameStatus;
}) {
  return (
    <div className="relative">
      <AtSign className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-gray-400 pointer-events-none" />
      ...
      <div className="absolute right-3 top-1/2 -translate-y-1/2">
        ...
        {status === 'available' && <Check className="h-4 w-4 text-brand-500" />}
        {status === 'taken' && <X className="h-4 w-4 text-coral-500" />}
        {status === 'invalid' && <X className="h-4 w-4 text-coral-500" />}
      </div>
    </div>
  );
}
...
function TermsCheckbox({
  ...
  t: (key: string) => string; accepted: boolean; onChange: (v: boolean) => void;
}) {
  return (
    <label className="flex items-start gap-2.5 cursor-pointer">
      <input
      ...
      />
      <span className="text-xs text-gray-600 dark:text-gray-400">
        {t('signup.acceptTerms').split('Terms')[0]}
        <Link to="/terms" className="text-brand-500 hover:underline" onClick={(e) => e.stopPropagation()}>Terms</Link>
        {' '}
        <Link to="/privacy" className="text-brand-500 hover:underline" onClick={(e) => e.stopPropagation()}>Privacy</Link>
      </span>
    </label>
  );
}
...
function LeftPanel({ t, userCount }: { t: (key: string) => string; userCount: number | null }) {
  ...
  useEffect(() => {
    const interval = setInterval(() => setFeatureIdx((prev) => (prev + 1) % FEATURES.length), 3000);
    return () => clearInterval(interval);
  }, []);
  ...
  return (
    <div className="max-w-lg">
      <div className="flex items-center gap-3 mb-6">
        <SangamLogo size={56} />
        <h1 className="font-heading text-4xl font-extrabold bg-sangam-gradient bg-clip-text text-transparent">Sangam</h1>
      </div>
      ...
      <div className="bg-white/50 dark:bg-navy-200/50 backdrop-blur-md rounded-2xl p-5 border border-white/30 dark:border-navy-300/30 mb-6 min-h-[120px]">
        <div key={featureIdx} className="animate-fadeIn">
          <div className="flex items-center gap-3 mb-2">
            ...
            <h3 className="font-heading text-lg font-bold text-gray-900 dark:text-white">{t(`login.${feature.key}`)}</h3>
          </div>
          <p className="text-sm text-gray-600 dark:text-gray-400 ps-13">{t(`login.${feature.descKey}`)}</p>
        </div>
        <div className="flex gap-1.5 mt-4">
          {FEATURES.map((_, i) => (
            <div key={i} className={`h-1.5 rounded-full transition-all ${i === featureIdx ? 'w-6 bg-sangam-gradient' : 'w-1.5 bg-gray-300 dark:bg-navy-400'}`} />
          ))}
        </div>
      </div>
      <div className="flex items-center gap-3 text-gray-600 dark:text-gray-400">
        <div className="flex -space-x-2">
          {['A', 'R', 'P', 'S'].map((l, i) => (
            <div key={i} className="h-8 w-8 rounded-full border-2 border-[#f5f5f4] dark:border-[#0b1220] bg-sangam-gradient flex items-center justify-center text-white text-xs font-bold">{l}</div>
          ))}
        </div>
        <div className="text-sm">
          {userCount !== null ? (
            <><span className="font-bold text-gray-900 dark:text-white">{userCount.toLocaleString()}+</span> {t('login.usersOnSangam')}</>
          ) : (
